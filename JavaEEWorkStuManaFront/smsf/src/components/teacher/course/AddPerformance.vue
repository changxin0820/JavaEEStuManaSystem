<template>
  <div>
    ...
    <el-card>
      ...
      <el-row :gutter="20">
        <el-col :span="7">
          <el-input placeholder="请输入学号" v-model="ID" clearable @clear="clearWord"><el-button slot="append" icon="el-icon-search" @click="getPerformance"></el-button></el-input>
        </el-col>
        <el-col :span="4">
          <el-button type="primary" @click="dialogVisible=true">导入成绩</el-button>
        </el-col>
      </el-row>
      <!--成绩列表区域-->
    ...
    export default {
        ...
        methods: {
            ...
            getPerformance() {
                var that = this;
                this.$axios.post(
                    '/api/performance/selectReportCardById',
                    this.$qs.stringify({sid:this.ID})
            ...
            addScore(){
                var that = this;
                this.$axios.post(
                    '/api/performance/insertScore',
                    this.$qs.stringify({sid:this.addForm.sid,
